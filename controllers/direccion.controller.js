...
self.getAll = async function (req, res, next) {
  try {
    ...
    if (!user) {
      console.log("Usuario no encontrado for email:", userEmail);
      return res.status(404).json({ message: "Usuario no encontrado" });
    }

    const userId = user.id;    const data = await direccion.findAll({ where: { usuarioId: userId } });
    console.log("Direcciones found:", data.length);
...
self.create = async function (req, res, next) {
  try {
    ...
    const userId = user.id;    const body = req.body;
